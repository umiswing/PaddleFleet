    ...
    def forward(
        ...
        if self.full_recompute or (not has_recovered()):
            ...
            outputs = recompute(
                ...
                # Plain int, and it must be a declared parameter of
                # _forward_impl: paddle's reentrant recompute binds kwargs
                # against the callee signature and flattens them into positional
                # args, so anything landing in **kwargs would be replayed as a
                # surplus positional and raise TypeError.
                **docmask_meta_kwargs,
                **offload_kwargs,
            )
        else:
