...
from .bwd_barriers import NamedBarrierBwd
from .bwd_tile_scheduler import (
    ParamsBase,
    SingleTileScheduler,
...
from .seqlen import SeqlenInfoQK
...
def _elem_pointer_packed_mh_i64(
    ...
    *,
    ...
    linear_idx = cutlass.Int64(m_idx) * cutlass.Int64(num_head) + cutlass.Int64(
        h_idx
    )
...
class _FlashAttentionDSABackwardPreprocessSm90:
    ...
    def __call__(
        ...
        mAttnSink: cute.Tensor | None,
        mdSink: cute.Tensor | None,
...
class FlashAttentionDSABackwardSm90:
    ...
    def __init__(
        ...
        assert qhead_per_kvhead > 1, (
            "This version only supports MQA/GQA (qhead_per_kvhead > 1)"
        )
        ...
        self.hdim_chunk = min(
            64, self.tile_hdim
        )  # hdim chunking for dKV GEMM3/5
        self.N_hdim_chunks = self.tile_hdim // self.hdim_chunk
        self.hdim_chunk_dq = min(
            128, self.tile_hdim
        )  # hdim chunking for dQ GEMM4 (each WG does 2 quarter GEMMs)
        ...
        self.hdim_chunk_dq_wg1_1 = (
            self.tile_hdim - self.hdim_chunk_dq * 3
            if not self.same_hdim_kv
            else self.hdim_chunk_dq
        )

        # K=V or V=K[0:d_v] constraints
        assert self.tile_hdim >= self.tile_hdimv, (
            "tile_hdim must be >= tile_hdimv"
        )
        assert self.num_mma_warp_groups == 2, (
            "WG-split mode requires 2 MMA warp groups"
        )
        assert self.tile_hdim % self.hdim_chunk == 0

    @staticmethod
    def can_implement(
        dtype, head_dim, head_dim_v, tile_m, tile_n, KV_stage, num_threads
    ) -> bool:
        if dtype not in [cutlass.Float16, cutlass.BFloat16]:
            return False
    ...
    def _get_shared_storage_cls(self):
        ...
        sP_struct = cute.struct.Align[
            cute.struct.MemRange[self.dtype, cosize_sPdS], 1024
        ]
        sdS_struct = cute.struct.Align[
            cute.struct.MemRange[self.dtype, cosize_sPdS], 1024
        ]
    ...
    def __call__(
        ...
        self._check_type(
            *(t.element_type for t in (mQ, mKV, mdO, mLSE, mdPsum, mdQ, mdKV))
        )
        ...
        def _assume_strides(t):
            ...
            return cute.make_tensor(
                t.iterator, cute.make_layout(t.shape, stride=tuple(new_strides))
            )
        ...
        mQ, mKV, mdO, mdQ = [
            select(t, layout_transpose) for t in (mQ, mKV, mdO, mdQ)
        ]
        ...
        self.tma_copy_bytes = {
            name: cute.size_in_bytes(
                mX.element_type, cute.select(layout, mode=[0, 1])
            )
    ...
    def kernel(
        ...
        sKV_q0 = storage.sKV.get_tensor(
            sKV_quarter_layout.outer, swizzle=sKV_quarter_layout.inner
        )
        q_elems_128 = self.tile_n * self.hdim_chunk_dq
        sKV_q1 = cute.make_tensor(
            sKV_q0.iterator + 1 * q_elems_128, sKV_quarter_layout.outer
        )
        sKV_q2 = cute.make_tensor(
            sKV_q0.iterator + 2 * q_elems_128, sKV_quarter_layout.outer
        )
        sKV_q3_192 = cute.make_tensor(
            sKV_q0.iterator + 3 * q_elems_128, sKV_192_layout.outer
        )
        ...
        sQ_half_first = storage.sQ.get_tensor(
            sQ_half_layout.outer, swizzle=sQ_half_layout.inner
        )
        ...
        sQ_q0 = storage.sQ.get_tensor(
            sQ_quarter_layout.outer, swizzle=sQ_quarter_layout.inner
        )
        sQ_q1 = cute.make_tensor(
            sQ_q0.iterator + self.tile_m * 128, sQ_quarter_layout.outer
        )
        sQ_q2 = cute.make_tensor(
            sQ_q0.iterator + self.tile_m * 256, sQ_quarter_layout.outer
        )
        sQ_q3 = cute.make_tensor(
            sQ_q0.iterator + self.tile_m * 384, sQ_quarter_layout.outer
        )
        # 192-col SMEM view for WG1 epilogue R2S of acc_dQ_3 (cols [384:576])
        sQ_192_epi = cute.make_tensor(
            sQ_q0.iterator + self.tile_m * 384, sKV_192_layout.outer
        )
    ...
    def mma_wg0(
        ...
        tSrQ, tSrKV = mma_partition_fragment_AB(
            wg_mma_SdP, sQ, sKV, self.SdP_swapAB
        )
        # GEMM2: dP = dO @ V^T (SS), K dim = tile_hdimv (512)
        tdPrdO, tdPrV = mma_partition_fragment_AB(
            wg_mma_SdP, sdO, sV, self.SdP_swapAB
        )
        ...
        _, tdQrKVt_q0 = mma_partition_fragment_AB(
            wg_mma_dQ, None, sKVt_q0, self.dQ_swapAB
        )
        _, tdQrKVt_q1 = mma_partition_fragment_AB(
            wg_mma_dQ, None, sKVt_q1, self.dQ_swapAB
        )
        ...
        tPsP = smem_thr_copy_PdS.partition_D(
            sP if const_expr(not self.SdP_swapAB) else transpose_view(sP)
        )
        tdSsdS = smem_thr_copy_PdS.partition_D(
            sdS if const_expr(not self.SdP_swapAB) else transpose_view(sdS)
        )
        ...
        tLSEsLSE = make_acc_tensor_mn_view(thr_mma_SdP.partition_C(sLSE_mma))[
            LSEslice
        ]
        tLSEsdPsum = make_acc_tensor_mn_view(
            thr_mma_SdP.partition_C(sdPsum_mma)
        )[LSEslice]
        ...
        # A zero-length sparse row skips every dQ GEMM.  The epilogue still
        # runs, so initialize the accumulators explicitly to make that row
        # produce a deterministic zero gradient instead of uninitialized data.
        acc_dQ_0.fill(0.0)
        acc_dQ_1.fill(0.0)
        ...
        while work_tile.is_valid_tile:
            ...
            if const_expr(self.have_topk_length):
                ...
                topK = self.max_topk
            # A zero-topK tile skips every dQ GEMM. Reset the accumulators
            # before entering the epilogue; the epilogue also forces its
            # register-to-shared source to zero because CTA shared memory can
            # contain data from a previously completed work tile.
            if topK == 0:
                acc_dQ_0.fill(0.0)
                acc_dQ_1.fill(0.0)
            ...
            gdO = cute.local_tile(
                mdO_cur, (self.tile_m, self.tile_hdimv), (m_block, 0)
            )
            ...
            if warp_idx_in_wg == 0:
                with cute.arch.elect_one():
                    cute.arch.mbarrier_arrive_and_expect_tx(
                        mbar_QdO_ptr,
                        self.tma_copy_bytes["Q"] + self.tma_copy_bytes["dO"],
                    )
            ...
            # A zero-length row has no n-block at all.  In particular, do not
            # call _wg0_one_n_block with n_block == -1: that would publish
            # sP_ready/sdS_ready while WG1 has correctly skipped its own
            # n-block loop, leaving WG0 waiting on a barrier WG1 never reaches.
            if topK > 0:
                # first n_block
    ...
    def scatter_dkv_atomic(
        ...
        chunk_idx: cutlass.Constexpr[
            int
        ],  # which hdim chunk (0..N_hdim_chunks-1)
        ...
        for r in cutlass.range_constexpr(nrow):
            ...
            if global_topk_row < topK:
                ...
                # The top-k producer normally emits a valid prefix followed
                # by -1 padding.  Keep the guard even in the bounded
                # specialization so malformed/interior invalid entries are
                # skipped rather than becoming negative dKV addresses.
                should_accumulate_dkv = global_kv_row >= 0
                if should_accumulate_dkv:
                    row_base = (
                        global_kv_row * self.tile_hdim
                    ...
                    for c4 in cutlass.range_constexpr(ncol // 4):
                        ...
                        c = const_expr(c4 * 4)
                        target_ptr = (
                            mdKVaccum_cur.iterator + row_base + fake_offset
                        )
                        atomic_add_fp32x4(
                            acc_mn[r, c + 0],
                            acc_mn[r, c + 1],
    ...
    def _wg0_one_n_block(
        ...
        for i in cutlass.range_constexpr(cute.size(tdKVrdS)):
            tdKVrdS_scaled[i] = (tdKVrdS[i].to(Float32) * softmax_scale).to(
                self.dtype
            )
        ...
        cute.copy(
            smem_thr_copy_PdS, tdSrdS, tdSsdS[None, None, None, smem_idx_PdS]
        )
        cute.arch.fence_view_async_shared()
        cute.arch.barrier_arrive(
    ...
    def mma_wg1(
        ...
        tdKVrPt, _ = mma_partition_fragment_AB(
            wg_mma_dKV, sPt, None, self.dKV_swapAB
        )
        # GEMM5 A: dS^T (MN-major transpose view)
        sdSt = transpose_view(sdS)
        tdKVrdSt, _ = mma_partition_fragment_AB(
            wg_mma_dKV, sdSt, None, self.dKV_swapAB
        )
        ...
        tdQrsdS, tdQrKVt_q2 = mma_partition_fragment_AB(
            wg_mma_dQ, sdS, sKVt_q2, self.dQ_swapAB
        )
        # G4_half_3: partition with 192-col tiled_mma (or 128-col for d=512, same layout)
        tdQrsdS_192, tdQrKVt_q3_192 = mma_partition_fragment_AB(
            wg_mma_dQ_192, sdS, sKVt_q3_192, self.dQ_swapAB
        )
        ...
        while work_tile.is_valid_tile:
            ...
            if const_expr(self.have_topk_length):
                ...
                topK = self.max_topk
            # Keep zero-topK dQ tiles independent from the previous work tile;
            # WG1 also skips all dQ GEMMs in this case.
            if topK == 0:
                acc_dQ_2.fill(0.0)
                acc_dQ_3.fill(0.0)
            n_block_max = (topK + self.tile_n - 1) // self.tile_n
            ...
            while n_block >= 0:
                ...
                sdO_c0 = cute.make_tensor(
                    sdO.iterator + 0 * chunk_elems, sdO_chunk_layout.outer
                )
                sQ_c0 = cute.make_tensor(
                    sQ.iterator + 0 * chunk_elems, sQ_chunk_layout.outer
                )
                _, tdKVrdOt_c0 = mma_partition_fragment_AB(
                    wg_mma_dKV, None, transpose_view(sdO_c0), self.dKV_swapAB
                )
                _, tdKVrQt_c0 = mma_partition_fragment_AB(
                    wg_mma_dKV, None, transpose_view(sQ_c0), self.dKV_swapAB
                )
                ...
                sdO_c1 = cute.make_tensor(
                    sdO.iterator + 1 * chunk_elems, sdO_chunk_layout.outer
                )
                sQ_c1 = cute.make_tensor(
                    sQ.iterator + 1 * chunk_elems, sQ_chunk_layout.outer
                )
                _, tdKVrdOt_c1 = mma_partition_fragment_AB(
                    wg_mma_dKV, None, transpose_view(sdO_c1), self.dKV_swapAB
                )
                _, tdKVrQt_c1 = mma_partition_fragment_AB(
                    wg_mma_dKV, None, transpose_view(sQ_c1), self.dKV_swapAB
                )
                ...
                sdO_c2 = cute.make_tensor(
                    sdO.iterator + 2 * chunk_elems, sdO_chunk_layout.outer
                )
                sQ_c2 = cute.make_tensor(
                    sQ.iterator + 2 * chunk_elems, sQ_chunk_layout.outer
                )
                _, tdKVrdOt_c2 = mma_partition_fragment_AB(
                    wg_mma_dKV, None, transpose_view(sdO_c2), self.dKV_swapAB
                )
                _, tdKVrQt_c2 = mma_partition_fragment_AB(
                    wg_mma_dKV, None, transpose_view(sQ_c2), self.dKV_swapAB
                )
                ...
                sdO_c3 = cute.make_tensor(
                    sdO.iterator + 3 * chunk_elems, sdO_chunk_layout.outer
                )
                sQ_c3 = cute.make_tensor(
                    sQ.iterator + 3 * chunk_elems, sQ_chunk_layout.outer
                )
                _, tdKVrdOt_c3 = mma_partition_fragment_AB(
                    wg_mma_dKV, None, transpose_view(sdO_c3), self.dKV_swapAB
                )
                _, tdKVrQt_c3 = mma_partition_fragment_AB(
                    wg_mma_dKV, None, transpose_view(sQ_c3), self.dKV_swapAB
                )
                ...
                sdO_c4 = cute.make_tensor(
                    sdO.iterator + 4 * chunk_elems, sdO_chunk_layout.outer
                )
                sQ_c4 = cute.make_tensor(
                    sQ.iterator + 4 * chunk_elems, sQ_chunk_layout.outer
                )
                _, tdKVrdOt_c4 = mma_partition_fragment_AB(
                    wg_mma_dKV, None, transpose_view(sdO_c4), self.dKV_swapAB
                )
                _, tdKVrQt_c4 = mma_partition_fragment_AB(
                    wg_mma_dKV, None, transpose_view(sQ_c4), self.dKV_swapAB
                )
                ...
                sdO_c5 = cute.make_tensor(
                    sdO.iterator + 5 * chunk_elems, sdO_chunk_layout.outer
                )
                sQ_c5 = cute.make_tensor(
                    sQ.iterator + 5 * chunk_elems, sQ_chunk_layout.outer
                )
                _, tdKVrdOt_c5 = mma_partition_fragment_AB(
                    wg_mma_dKV, None, transpose_view(sdO_c5), self.dKV_swapAB
                )
                _, tdKVrQt_c5 = mma_partition_fragment_AB(
                    wg_mma_dKV, None, transpose_view(sQ_c5), self.dKV_swapAB
                )
                ...
                sdO_c6 = cute.make_tensor(
                    sdO.iterator + 6 * chunk_elems, sdO_chunk_layout.outer
                )
                sQ_c6 = cute.make_tensor(
                    sQ.iterator + 6 * chunk_elems, sQ_chunk_layout.outer
                )
                _, tdKVrdOt_c6 = mma_partition_fragment_AB(
                    wg_mma_dKV, None, transpose_view(sdO_c6), self.dKV_swapAB
                )
                _, tdKVrQt_c6 = mma_partition_fragment_AB(
                    wg_mma_dKV, None, transpose_view(sQ_c6), self.dKV_swapAB
                )
                ...
                sdO_c7 = cute.make_tensor(
                    sdO.iterator + 7 * chunk_elems, sdO_chunk_layout.outer
                )
                sQ_c7 = cute.make_tensor(
                    sQ.iterator + 7 * chunk_elems, sQ_chunk_layout.outer
                )
                _, tdKVrdOt_c7 = mma_partition_fragment_AB(
                    wg_mma_dKV, None, transpose_view(sdO_c7), self.dKV_swapAB
                )
                _, tdKVrQt_c7 = mma_partition_fragment_AB(
                    wg_mma_dKV, None, transpose_view(sQ_c7), self.dKV_swapAB
                )
                ...
                if const_expr(not self.same_hdim_kv):
                    ...
                    sQ_c8 = cute.make_tensor(
                        sQ.iterator + 8 * chunk_elems, sQ_chunk_layout.outer
                    )
                    _, tdKVrQt_c8 = mma_partition_fragment_AB(
                        wg_mma_dKV, None, transpose_view(sQ_c8), self.dKV_swapAB
                    )
            ...
            # With topK == 0 WG0 does not enter its n-block loop and therefore
            # cannot observe the usual sdS_consumed arrival.  Signal it here
            # before the dQ epilogue; the dQ accumulators were zero-initialized
            # above, so this row returns exactly zero.
            if topK == 0:
                cute.arch.barrier_arrive(
                    barrier_id=int(NamedBarrierBwd.sdS_consumed),
                    number_of_threads=self.num_mma_threads,
    ...
    def epilogue_dQ_wg1(
        ...
        smem_copy_atom_128 = cute.make_copy_atom(
            cute.nvgpu.warp.StMatrix8x8x16bOp(
                transpose=self.dQ_swapAB, num_matrices=4
            ),
            self.dtype,
        )
        smem_thr_copy_128 = cute.make_tiled_copy_C(
            smem_copy_atom_128, tiled_mma_dQ_128
        ).get_slice(wg_tidx)
        ...
        sdQ2 = (
            sQ_q2 if const_expr(not self.dQ_swapAB) else transpose_view(sQ_q2)
        )
        ...
        smem_thr_copy_192 = cute.make_tiled_copy_C(
            smem_copy_atom_192, tiled_mma_dQ_192
        ).get_slice(wg_tidx)
        ...
        sdQ3_192 = (
            ...
            else transpose_view(sQ_192_epi)
        )
        ...
        if warp_idx_in_wg == 0:
            ...
            gdQ_half = cute.local_tile(
                mdQ_cur, (self.tile_m, 256), (m_block, 1)
            )  # col_block=1 → offset 256
            with cute.arch.elect_one():
        ...
        if const_expr(not self.same_hdim_kv):
            if warp_idx_in_wg == 0:
                ...
                gdQ_64 = cute.local_tile(
                    mdQ_64_cur, (self.tile_m, 64), (m_block, 8)
                )  # col_block=8 → offset 512
                with cute.arch.elect_one():
    ...
    def epilogue_dQ(
        ...
        sdQ0 = (
            sQ_q0 if const_expr(not self.dQ_swapAB) else transpose_view(sQ_q0)
        )
        ...
        sdQ1 = (
            sQ_q1 if const_expr(not self.dQ_swapAB) else transpose_view(sQ_q1)
        )
        ...
        cute.arch.fence_view_async_shared()
        epilogue_bar_id = (
            int(NamedBarrierBwd.Epilogue_WG0)
            if wg_idx == 0
            else int(NamedBarrierBwd.Epilogue_WG1)
        )
        cute.arch.barrier(
...
class _FlashAttentionDSABackwardPostprocessSm90:
    ...
    def _smem_to_gmem(
        ...
        """
        ELEMS_PER_THR = const_expr(
            self.tile_n * self.hdim_chunk // self.num_threads
        )
