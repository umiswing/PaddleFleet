...
class NamedBarrierBwd:
    ...
    sdS_consumed = (
        4  # WG1 -> WG0: all GEMM5/G4 read sdS done + sQ safe for epilogue
    )
    G4_half_ready = (
        5  # WG1 -> WG0: WG1 G4_half committed, WG0 can issue G4_half
    )
    ...
    WG0_producer_sync = (
        8  # WG0 internal sync — warp0 TMA KV done, all 128 threads can proceed
    )
    ...
    DenseConsumer0Sync_3WG = (
        10  # WG0 internal 128-thread sync (per-iter sReduce fence)
    )
    DenseConsumer1Sync_3WG = (
        11  # WG1 internal 128-thread sync (per-iter sReduce fence)
    )
    QueryWeightsReady3WG = (
        12  # 384-thread: producer -> consumers, Q + Weights / LSE in SMEM
    )
    TileComplete3WG = (
        13  # 384-thread: consumers -> producer, tile epilogue done
    )
    ConsumersDone3WG = (
        14  # 256-thread: WG0 + WG1, partial state exchanged in sReduce
    )
    ProducerHTileSync_3WG = (
        15  # 128-thread producer-WG private sync at h_tile boundary
    )
