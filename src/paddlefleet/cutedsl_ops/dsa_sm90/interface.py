...
def _patch_cutlass_nvgpu():
    """Normalize OperandMajorMode across installed CUTLASS DSL versions."""
    try:
...
from .tensor_conversion import to_cute_tensor
...
def flash_attn_bwd_sm90(
    q: paddle.Tensor,
    kv: paddle.Tensor,  # K=V unified tensor
    out: paddle.Tensor,
    dout: paddle.Tensor,
    lse: paddle.Tensor,
    attn_sink: paddle.Tensor | None = None,
    softmax_scale: float | None = None,
    dq: paddle.Tensor | None = None,
    dkv: paddle.Tensor | None = None,
    d_sink: paddle.Tensor | None = None,
    topk_idxs: paddle.Tensor = None,
    topk_length: paddle.Tensor = None,
    ...
    topk4 = (
        topk_idxs.unsqueeze(0) if topk_idxs is not None else None
    )  # (1, total_S_q, TopK)
    tlen4 = (
        topk_length.unsqueeze(0) if topk_length is not None else None
    )  # (1, total_S_q)
    ...
    q4, kv4, out4, dout4, lse4 = [
        maybe_contiguous(t) for t in (q4, kv4, out4, dout4, lse4)
    ]
    ...
    seqlen_q_rounded = (
        (seqlen_q + m_block_size - 1) // m_block_size * m_block_size
    )
    seqlen_k_rounded = (
        (seqlen_k + n_block_size - 1) // n_block_size * n_block_size
    )

    assert q4.dtype in [paddle.float16, paddle.bfloat16]
    assert q4.dtype == kv4.dtype == out4.dtype == dout4.dtype
    assert lse4.dtype == paddle.float32
    assert all(t.place.is_gpu_place() for t in (q4, kv4, out4, dout4, lse4))
    if attn_sink is not None:
        assert attn_sink.dtype == paddle.float32
        assert attn_sink.shape == (num_head,)
        assert attn_sink.place.is_gpu_place()
    ...
    # Resolve the execution stream before any explicit tensor initialization.
    current_stream = current_stream or current_cu_stream()

    # Allocate flat output buffers, then view as 4D for the kernel
    if dq is None:
        # Zero-topK rows skip the dQ epilogue entirely.  Start from zero so
        # those rows return an actual zero gradient instead of exposing
        # uninitialized output or stale shared-memory data.
        dq = paddle.zeros_like(q)  # (total_S_q, H, D)
    if dkv is None:
        dkv = paddle.empty([total_S_kv, head_dim], dtype=kv.dtype)
    ...
    if return_d_sink:
        assert attn_sink is not None, (
            "attn_sink is required when requesting d_sink"
        )
        if d_sink is None:
            d_sink = paddle.zeros_like(attn_sink)
        else:
            d_sink.fill_(0)
    ...
    dpsum = paddle.empty(
        [batch_size, seqlen_q_rounded, num_head], dtype=paddle.float32
    )
    lse_log2 = paddle.empty(
        [batch_size, seqlen_q_rounded, num_head], dtype=paddle.float32
    )
    dkv_accum = paddle.zeros(
        [
            batch_size,
            num_head_kv,
    ...
    dtype = paddle2cute_dtype_map[q4.dtype]
    arch = 90
    num_threads = 256
    ...
    assert topk4.dtype == paddle.int32
    have_topk_length = tlen4 is not None
    if have_topk_length:
        assert tlen4.dtype == paddle.int32
    else:
        tlen4 = paddle.empty([1], dtype=paddle.int32)
