    ...
    if const_expr(arch < 90 or element_type.width != 16):
        ...
        return cute.make_copy_atom(
            cute.nvgpu.warp.StMatrix8x8x16bOp(
                transpose=transpose, num_matrices=4
            ),
            element_type,
        )
...
def make_acc_tensor_mn_view(
    acc: cute.Tensor, transpose: bool = False
) -> cute.Tensor:
...
def elem_pointer_packed_i64(
    ...
    *,
    ...
    linear_idx = cutlass.Int64(h_idx) * cutlass.Int64(seqlen_q) + cutlass.Int64(
        m_idx
    )
...
def cvt_f16(src: cute.Tensor, dst_or_dtype):
    ...
    if const_expr(isinstance(dst_or_dtype, type)):
        ...
        assert cute.size(src.shape) % 2 == 0, (
            "src must have an even number of elements"
        )
        ...
        for i in cutlass.range_constexpr(cute.size(dst_i32)):
            dst_i32[i] = cvt_f16x2_f32(
                src[2 * i], src[2 * i + 1], dst.element_type
            )
