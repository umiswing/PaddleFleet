...
#
# 用法:
#   bash run.sh <scale> <stage> [dense|slashmla] [额外的 train.sh 参数...]
#     scale : 0p6B | 1p7B | 10B
#     stage : 8k | 32k | 128k
#   例:
#     bash run.sh 10B 8k
...
# Keep the historical two-argument form working: when the third argument
# starts with "-", it is a train.sh option rather than an attention mode.
if [[ "$mode" == -* ]]; then
  mode="dense"
  shift 2
else
...
case "$mode" in
  ...
  slashmla)
    ...
    ;;
  *)
    echo "错误: mode 必须是 dense|slashmla (给的是 '$mode')"
    exit 1
    ;;
esac
